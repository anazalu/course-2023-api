...
export async function request(context, method, path, body = undefined, auth = true, asserts = {statusCode : 200},  host = undefined, customHeaders = undefined) {
    ...
    switch (method) {
        case 'GET':
            ...
            await performValidation(responseBody, asserts, context, method, path, headers, response, body)
...
async function validateExpectedTypes(body, fields, context, method, path, headers, response, requestBody) {
    fields.every(field => {
        ...
        try {
            ...
            assert.fail(actualValue, field.type, `Expected type was ${field.type}, but received ${typeOfReceived} for value ${actualValue}`)
        }
    })
}
...
async function validateExpectedValuesInArrayOfObjects(body, fields, context, method, path, headers, response, requestBody, idKey = '_id', value) {
    const objectToValidate = body.find(item => item[idKey] === value)

    if (!objectToValidate) {
        assert.fail(idKey, value, `object with key ${idKey} and value ${value} not found`)
    }
...
async function performValidation(responseBody, asserts, context, method, path, headers, response, body) {
    ...
    if (asserts.expectedValuesInArrayOfObjects) {
        await validateExpectedValuesInArrayOfObjects(responseBody, asserts.expectedValuesInArrayOfObjects.fields, context, method, 
            path, headers, response, body, asserts.expectedValuesInArrayOfObjects.idKey, asserts.expectedValuesInArrayOfObjects.value)
    }
}
